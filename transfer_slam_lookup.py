...
class CLIPFeatureExtractor:
    ...
    def generate_class_id_map(self, image, masks, class_indices, classes):
        ...
        # Use the ID for 'others' as the default
        others_class_id = self.classname_to_id['others']
        class_id_map = np.full((H, W), fill_value=others_class_id, dtype=np.int32)

        for i, mask in enumerate(masks):
            class_idx = class_indices[i]

            # Skip invalid or N/A classes
            if class_idx >= len(classes) or classes[class_idx] == "N/A":
                continue

            class_name = classes[class_idx]
            class_id = self.classname_to_id.get(class_name, others_class_id)

            class_id_map[mask > 0] = class_id
...
classname_to_id = {v: k for k, v in class_names.items()}
    ...
    for cam in tqdm(splats["slam_positions"], desc="Feature backprojection"):
            ...
            with torch.no_grad():
                ...
                if masks.numel() > 0:  # Ensure masks are not empt 
                    ...
                    clip_id_map = clip_extractor.generate_class_id_map(image, masks.squeeze(1), class_indices, class_names)
